...
const Window = ({ name, removeFromStack, children }: WindowProps) => {
  ...
  return <div
    className={
      ...
      ${isOpen
        ?
        ' opacity-100 ' + (
          (isFullSize ? 'z-30 fixed top-3 bottom-3 left-3 right-3 lg:bottom-10 lg:top-10 lg:right-10 lg:left-10' : ' mb-3 ') + (isMinimised ? ' max-h-12 ' : ' max-h-screen ')
        )
        :
  ...
  >
    ...
    />
    <div className={`px-12 py-6 flex flex-col overflow-y-auto ${isFullSize ? 'max-h-full' : ' min-h-fit  max-h-80 '} `}>
      {children}
    </div>
  </div>
  ...
  >
    ...
    >
      ...
      <button
        ...
        {isHoveredOver
          &&
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 12 12" strokeWidth="1.5" stroke="currentColor" className="w-2 h-2 scale-150 text-slate-800">
